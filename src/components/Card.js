...
function Card(props) {
  ...
  return (
    ...
    <div className='w-full h-full flex flex-wrap mt-6 p-7'>
        ...
        <p className='font-bold text-purple-900 text-[20px] mt-2 p-1 w-auto rounded-xl'>{price}</p>
        <h1 className='font-bold text-[1.1rem] pl-2 pt-3 pr-1'>{title}</h1>
        <p className='pt-1 pl-2 text-[16px]'>{param}</p>
        <button className='relative bottom-[-1rem] bg-sky-500 ease-in duration-75 w-[130px] h-[40px] rounded-3xl hover:bg-sky-400 font-bold text-stone-100 pl-1 pr-1' onClick={handleClick}>{button}</button>
        </div>
    </div>
    </>
  )
